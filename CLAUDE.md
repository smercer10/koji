...
## Commands
`zig build` (ReleaseFast) · `zig build test` (unit + shallow perft, warm ~4s — the turn gate) ·
`zig build test-slow` (deep perft, and the search tests at full depth) · `zig build bench` ·
`-Doptimize=Debug` · `-Dtunables` (SPSA knobs,
...
(`/sprt`, smoke match): a silent engine passes every other gate here — a suppressed `bestmove` once
lost 400 games out of 400 behind a green suite, a bit-identical bench and green CI (testlog 2026-08-16).
